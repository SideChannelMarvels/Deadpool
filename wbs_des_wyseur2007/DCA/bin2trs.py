...
#########################################################################
# bin2trs is a script converting binary execution traces                #
# created by Tracer2bin, or other tools using the same format,          #
# into traces compatible with Riscure Inspector.                        #
#                                                                       #
# Copyright (C) 2016                                                    #
...
for k,(tag, struct_input_sample, struct_output_sample, list_bits_per_output_sample) in trace_defs.iteritems():
    ...
    for filename in glob.glob('trace_%s_*' % tag):
        i,plain,cipher=filename[len('trace_%s_' % tag):-len('.bin')].split('_')
        if not blocksize:
        ...
        if not min_size or min_size > filesize:
            min_size = filesize
        traces_meta[int(i)] = [filename, int(plain, 16), int(cipher, 16)]
    with open('trs_%s_%i_%i.trs' % (k, n, min_size), 'wb') as trs:
        ...
        trs.write('\x44\x02' + struct.pack('<H', 2*blocksize))
        # End of header
        trs.write('\x5F\x00')
        for (filename, plain, cipher) in traces_meta.itervalues():
            # crypto data
            trs.write(('%0*X%0*X' % (2*blocksize, plain, 2*blocksize, cipher)).decode('hex'))
