...
def AesGetAllRoundKeys(
    ...
    if foundkey:
        p = 0  # null plaintext
        cint, _, _ = engine.doit(
            engine.goldendata, processinput(p, 16), lastroundkeys=[]
        )
        kr0 = phoenixAES.rewind(
            phoenixAES.int2bytes(cint),
            lastroundkeys,
            encrypt=encrypt,
